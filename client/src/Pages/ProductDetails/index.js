    ...
    if (user !== null) {
      ...
      if (reviews.review !== "") {
        ...
        postData("/api/productReviews/add", reviews).then((res) => {
          ...
          fetchDataFromApi(`/api/productReviews?productId=${id}`).then(
            (res) => {
              setreviewsData(res);
            }
          );
      ...
      postData(`/api/my-list/add/`, data).then((res) => {
        if (res.status !== false) {
          context.setAlertBox({
            open: true,
            error: false,
            msg: "Sản phẩm được thêm vào giỏ hàng",
          });
                      ...
                      <tbody>
                        ...
                        <tr className="pa_size">
                          ...
                          <td>
                            {productData?.productWeight?.map((item, index) => {
                              return (
                                <li className="list-inline-item shadow p-2  bg-blue text-white ">
                                  <span>{item}</span>
                                </li>
                              );
                            })}
