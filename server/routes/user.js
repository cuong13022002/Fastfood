...
router.post(`/signup`, async (req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            res.json({status:'FAILED', msg: "Người dùng đã tồn tại với email này!" })
            return;
        }

        if (existingUserByPh) {
            res.json({status:'FAILED', msg: "Người dùng đã tồn tại với số điện thoại này!" })
            return;
        }
        ...
        console.log(error);
        res.json({status:'FAILED', msg:"Đã xảy ra sự cố"});
        return;
    }
})
...
router.post(`/signin`, async (req, res) => {
    ...
    try{
        ...
        if(!matchPassword){
            return res.status(400).json({error:true,msg:"Sai mật khẩu !"})
        }
       ...
       return res.status(200).send({
        ...
        res.status(500).json({error:true,msg:"Đã xảy ra sự cố"});
        return;
    }
...
router.put(`/changePassword/:id`, async (req, res) => {
   ...
   // console.log(req.body)
    ...
    if(!matchPassword){
        res.status(404).json({error:true,msg:"Mật khẩu hiện tại sai"})
    }else{
        ...
        if(!user)
        return res.status(400).json({error:true,msg:'Người dùng không  được cập nhật!'})
...
router.get('/:id', async(req,res)=>{
    ...
    if(!user) {
        res.status(500).json({message: 'Không tìm thấy người dùng có ID đã cho.'})
    } else{
...
router.put('/:id',async (req, res)=> {
    ...
    if(!user)
    return res.status(400).send('người dùng không được cập nhật!')
